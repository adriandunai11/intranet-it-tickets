...
class ExpiredTicketsNotificationService
{
    private const FROM_EMAIL = 'intranet@miellgroup.com';
    private const FROM_NAME = 'MIELL Group - Intranet';
    private const TEMPLATE_CODE = 'it_tickets_expired_tickets';
    ...
    {
        $expiredTickets = $this->ticketsModel->getExpiredTickets(self::ACTIVE_STATUSES);

        if (empty($expiredTickets)) {
            $this->cliWrite('Expired tickets: 0', 'yellow');
            return true;
        }

        $this->cliWrite('Expired tickets: ' . count($expiredTickets), 'green');

        $groupedTickets = $this->groupTicketsByResponsibleAndArea($expiredTickets);
